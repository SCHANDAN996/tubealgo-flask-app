...
def upload_video(credentials, video_filepath, metadata):
    """
    Uploads a video from a given filepath to YouTube.
    This function is designed to be called from a background task.
    It does NOT handle file cleanup; the caller is responsible.
    """
    try:
        youtube = build('youtube', 'v3', credentials=credentials)
        body = {
...
def set_video_thumbnail(credentials, video_id, image_filepath):
    """
    Sets a thumbnail for a video from a given image filepath.
    ...
    try:
        ...
        logging.error(f"Could not set thumbnail for video {video_id} from path {image_filepath}: {e}")
        return {'error': str(e)}