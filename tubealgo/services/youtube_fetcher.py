...
# --- यह नया हेल्पर फंक्शन यहाँ जोड़ा गया है ---
def _create_video_objects(video_items):
    """
    YouTube API से मिले वीडियो आइटम्स को प्रोसेस करके हमारे स्टैंडर्ड वीडियो ऑब्जेक्ट्स बनाता है।
    यह डुप्लीकेट कोड को हटाने के लिए बनाया गया है।
    """
    videos = []
...
def get_latest_videos(channel_id, max_results=20, page_token=None):
    ...
    try:
        ...
        # अब हम डुप्लीकेट लूप की जगह सीधे हेल्पर फंक्शन को कॉल करेंगे
...
def analyze_channel(channel_input):
    ...
    # --- यहाँ बदलाव किया गया है: print को logging.debug से बदला गया ---
    logging.debug(f"--- (analyze_channel): Received input '{channel_input}' ---")
    ...
    try:
        ...
        if found_id and found_id.startswith('UC'):
            channel_id = found_id
            logging.debug(f"(analyze_channel): Resolved to channel_id '{channel_id}' via direct lookup.")
            ...
            if search_response.get('items'):
                ...
                logging.debug(f"(analyze_channel): Resolved to channel_id '{channel_id}' via handle '{found_id}'.")
            ...
            if search_response.get('items'):
                channel_id = search_response['items'][0]['id']['channelId']
                logging.debug(f"(analyze_channel): Resolved to channel_id '{channel_id}' via search.")

        if not channel_id:
            return {'error': f"No channel found for '{channel_input}'."}
        
        cache_key = f"channel_analysis_v5:{channel_id}"
        logging.debug(f"(analyze_channel): Using cache_key '{cache_key}'")

        cached_data = get_from_cache(cache_key)
        if cached_data:
            logging.debug("(analyze_channel): Cache HIT!")
            return cached_data
        
        logging.debug("(analyze_channel): Cache MISS! Fetching from API.")
        ...
        set_to_cache(cache_key, result, expire_hours=24)
        logging.debug("--- END DEBUG (analyze_channel) ---")
        return result
