...
from tubealgo.models import (
    User, Coupon, SubscriptionPlan, SiteSetting, Payment, ApiCache, 
    get_config_value, APIKeyStatus, SystemLog, YouTubeChannel, Competitor, DashboardCache
)
...
def user_details(user_id):
    ...
    if user.subscription_plan != 'free' and user.subscription_end_date:
        if user.subscription_end_date > datetime.utcnow():
            remaining_days = (user.subscription_end_date - datetime.utcnow()).days
        else:
            remaining_days = 0
...
def update_subscription(user_id):
    ...
    try:
        ...
        if user.subscription_end_date and user.subscription_end_date > datetime.utcnow():
            ...
            user.subscription_end_date = datetime.utcnow() + timedelta(days=days_to_add)
...
def reset_user(user_id):
    ...
    try:
        ...
        db.session.commit()
        flash(f'Account for {user.email} has been reset. All channel and competitor data has been cleared.', 'success')
    except Exception as e:
        db.session.rollback()
...
def upgrade_user(user_id):
    ...
    if new_plan in ['free', 'creator', 'pro']:
        ...
        if new_plan != 'free' and not user.subscription_end_date:
...
def site_settings():
    if request.method == 'POST':
        ...
        for key, value in form_data.items():
            secret_keys = ['OPENAI_API_KEY', 'YOUTUBE_API_KEYS', 'TELEGRAM_BOT_TOKEN', 'RAZORPAY_KEY_ID', 'RAZORPAY_KEY_SECRET', 'GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET']
...
def ai_settings():
    if request.method == 'POST':
        ...
        # --- बदलाव यहाँ है: नए प्रॉम्प्ट्स को डेटाबेस में सेव किया जा रहा है ---
    ...
    def get_model_display_info(model_name):
        ...
        if "pro" in model_name: return "Most powerful model for complex reasoning"
