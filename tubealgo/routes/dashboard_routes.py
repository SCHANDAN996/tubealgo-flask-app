...
def main_dashboard_data():
    ...
    # यदि कैश में डेटा है, तो उसे तुरंत लौटाएं
    if cache_entry and cache_entry.data:
        print(f"Dashboard Cache HIT for user {current_user.email}")
        return jsonify(cache_entry.data)
    
    # यदि कैश नहीं है (जैसे कि एक नया उपयोगकर्ता), तो एक बार के लिए लाइव डेटा लोड करें
    ...
    try:
        ...
        if creds:
            ...
            if 'error' not in videos_data:
                ...
                log_system_event("Dashboard Info: Could not fetch user videos with creds, falling back.", "INFO", {'user_id': current_user.id, 'error': videos_data['error']})
        ...
        if not all_user_videos:
            ...
            if 'error' in videos_data:
                log_system_event("Dashboard API Error: Fallback video fetch failed", "ERROR", {'user_id': current_user.id, 'error': videos_data['error']})
                return jsonify({'error': f"Could not load your videos: {videos_data['error']}"}), 500
        ...
        # नए डेटा को डेटाबेस में कैश करें
        if not cache_entry:
