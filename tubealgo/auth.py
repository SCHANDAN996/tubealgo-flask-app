...
def google_login():
    """Handles initial login/signup with Google."""
    redirect_uri = url_for('auth.google_callback', _external=True)
    flow = get_flow(LOGIN_SCOPES, redirect_uri)
...
@login_required
def connect_youtube():
    """Handles connecting a YouTube channel for an already logged-in user."""
    redirect_uri = url_for('auth.google_callback', _external=True)
    flow = get_flow(YOUTUBE_SCOPES, redirect_uri)
    authorization_url, state = flow.authorization_url(access_type='offline', include_granted_scopes='true', prompt='consent')
    session['state'] = state
    session['flow_type'] = 'youtube'
    session['oauth_scopes'] = YOUTUBE_SCOPES
    return redirect(authorization_url)

@auth.route('/callback')
def google_callback():
    """Single callback to handle both login and channel connect."""
