# Filepath: tubealgo/decorators.py
...
# NOTE: The import below has been corrected to point to youtube_manager.
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if feature == 'add_competitor' and (plan.competitors_limit != -1 and current_user.competitors.count() >= plan.competitors_limit):
                flash(f"You've reached the maximum of {plan.competitors_limit} competitors for your plan. Please upgrade.", "error")
                ...
                if plan.keyword_searches_limit != -1 and current_user.daily_keyword_searches >= plan.keyword_searches_limit:
                    flash(f"You've reached your daily limit of {plan.keyword_searches_limit} keyword searches. Please upgrade.", "error")
                    return redirect(url_for('core.pricing'))
                ...
                if plan.ai_generations_limit != -1 and current_user.daily_ai_generations >= plan.ai_generations_limit:
                    flash(f"You've reached your daily limit of {plan.ai_generations_limit} AI generations. Please upgrade.", "error")
                    return redirect(url_for('core.pricing'))
                current_user.daily_ai_generations += 1

            elif feature == 'discover_tools' and not plan.has_discover_tools:
                flash("The Discover tool is a premium feature. Please upgrade to access it.", "error")
                return redirect(url_for('core.pricing'))
